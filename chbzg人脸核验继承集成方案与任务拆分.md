...
> 当前可执行的接口约定、数据库设计、业务流程和逐项任务清单见
> [chbzg人脸核验接入实施方案与任务清单.md](chbzg人脸核验接入实施方案与任务清单.md)。
> 本文件保留前期双侧审计、设计依据和总体方案。
...
截至 2026-06-13，独立服务生产化 P0 主链路已经完成：

- 模板、verification session 和 proof 已使用 SQLite 持久化，单机重启不丢失。
- session/proof 已绑定人员、管理员、场景、业务事件、记录和动作。
- 已提供内部 API Key、短期上传令牌和生产域名严格 CORS。
- 已提供模板版本、旧模板自动吊销、统一结果码、`/ready` 和一次性 proof finalize。
- `/v1/verification-sessions/{session_id}/verify` 已直接给出综合判定并签发 proof；
  `/api/compare` 只保留 Demo 兼容用途。
- 双 MiniFASNet 与 InsightFace 模型已离线部署并完成生产预热。
- 已部署至 `https://faces.chbzg.com.cn`，systemd 服务和 HTTPS 健康检查正常。

当前独立服务仍有以下生产增强项：

- SQLite 只适合当前单实例；多实例前需要共享数据库/Redis 和分布式一致性设计。
- 尚未完成全局限流、排队、压力测试、结构化审计、指标和告警。
- 模板尚无独立人工吊销、批量迁移和失败重采管理接口。
- InsightFace `buffalo_l` 和 PAD 模型的生产使用/再分发许可仍需确认。
- 真实摄像头与攻击介质样本尚未完成现场阈值验收。
...
### 12.0 当前进度（2026-06-13）

状态说明：`完成` 表示已有代码和自动化/运行证据；`部分完成` 表示主链路可用但
仍缺生产增强；`未完成` 表示在当前本地仓库和本次生产部署中未找到实现证据。

| 编号 | 状态 | 当前结论 |
| --- | --- | --- |
| A1 | 完成 | SQLite 已实现 template/session/proof 状态和转换，并有测试 |
| A2 | 部分完成 | 已持久化且单机重启不丢失；未使用 Redis，未支持多实例共享状态 |
| A3 | 部分完成 | 模板版本和旧模板自动吊销已完成；独立吊销/管理/批量迁移未完成 |
| A4 | 完成 | subject、scene、event、record、action 绑定及短期上传令牌已完成 |
| A5 | 完成 | `/v1` verify 综合判定并签发 proof，业务无需调用 `/api/compare` |
| A6 | 部分完成 | API Key、严格 CORS、字段和请求大小限制已完成；全局限流未完成 |
| A7 | 部分完成 | 模型离线部署、预热和 ready 已完成；模型许可确认未完成 |
| A8 | 部分完成 | request ID 幂等、状态原子更新和系统错误重试已覆盖；排队/压测未完成 |
| A9 | 未完成 | 尚无完整结构化审计、指标、告警及模型/阈值策略版本追踪 |
| B1-B9 | 未完成 | `chbzg` 业务代码尚未开始接入，本次未修改线上业务项目 |
| C1-C5 | 未完成 | 尚未导出人员头像、批量建模、生成失败清单或提供重采入口 |
| D1 | 部分完成 | 独立服务 API、存储、绑定和 proof 自动化测试通过；业务契约测试待接入 |
| D2 | 未完成 | 登录与审核后端 Guard 尚未实现，无法开展完整防绕过测试 |
| D3 | 未完成 | 本人、非本人、照片、屏幕和视频真实样本记录为空 |
| D4 | 未完成 | 低配终端与网络波动现场测试尚未记录 |
| D5 | 部分完成 | 单实例持久化和生产重启状态已验证；并发/多实例一致性未验证 |
| D6 | 未完成 | `chbzg` 灰度、回滚和旧链路下线演练尚未开始 |

独立服务当前验证结果：

- 本地 Python 单元测试 27 项全部通过。
- Python/JavaScript 语法检查和前端静态检查通过。
- 生产 `https://faces.chbzg.com.cn/api/health` 和 `/api/ready` 均返回 HTTP 200。
- 生产服务 `face-verify.service` 为 active/enabled，监听 `127.0.0.1:8005`，
  当前发布版本为 Git 提交 `fdf788a`，检查时重启次数为 0。
- 上述结果证明接口、状态机和部署可用，不等同于真实人脸攻击样本已经验收。
