...
The test service listens on `127.0.0.1:19004` and reads/writes the test chbzg MySQL database through the DSN in `testing.env.example`. Nginx terminates HTTPS on the dedicated doctor face-verification port `9004` and proxies to `127.0.0.1:19004`. The existing business site on `443` is unchanged; only doctor login/verification responses should return a `verify_url` on port `9004`.
